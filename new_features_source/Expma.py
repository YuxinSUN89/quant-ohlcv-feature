...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Expma', is_sort_asc, n, arg)
    含义：Expma = Scale01(EMA(n) - EMA(4*n), n)
    示例：'factor_list': [
                            ('Expma', True, 10, 1),         # Expma_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    ema1 = df['收盘价_复权'].ewm(span=n, min_periods=1).mean()
    ema2 = df['收盘价_复权'].ewm(span=(4 * n), min_periods=1).mean()
    signal = ema1 - ema2
    # ========== 原始计算逻辑结束 ==========

    # 创建因子列
    df[f'Expma_{n}'] = scale_01(signal, n)
    factor_col = df[f'Expma_{n}']
