...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Micd', is_sort_asc, n, arg)
    含义：Mi = 收盘价 - 收盘价.shift(1), Dif = MA(MA(Mi,n),n) - MA(MA(Mi,n),2 * n)
        Micd = Dif - MA(Dif, n))
    示例：'factor_list': [
                            ('Micd', True, 20, 1),         # Micd_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    df['MI'] = df['收盘价_复权'] - df['收盘价_复权'].shift(1)
    ...
    # 创建因子列
    df[f'Micd_{n}'] = df['DIF'] / df['MICD']
    factor_col = df[f'Micd_{n}']
