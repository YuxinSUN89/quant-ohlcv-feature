...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('DDX', is_sort_asc, n, arg)
    含义：大单动向 DDX = (机构资金净流入 + 大户资金净流入) / 流通市值，DDX_n是指为DDX取n日均值，缺省为1，n=1时表示当日DDX
    示例：'factor_list': [
                            ('DDX', True, '', 1),      # DDX                          
                            ('DDX', True, 3, 1),       # DDX_3  
                        ]
    ...
    n = int(param) if param else 1      # 缺省值为1
    ...
    factor_col = df[f'DDX_Mean{n}']
    del df['机构资金净流入temp'], df['机构资金买入额'], df['机构资金卖出额'], df['大户资金净流入temp'], df['大户资金买入额'], df['大户资金卖出额'], df['DDX']
    # 创建包含指定因子的DataFrame
