...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('换手率Mean', is_sort_asc, n, arg)
    含义：换手率Mean = MA(成交额 / 流通市值, n)
    示例：'factor_list': [
                            ('换手率Mean', True, 20, 1),         # 换手率Mean_20
                        ]
    """
    # ======================== 参数处理 ===========================
    ...
    # ======================== 计算因子 ===========================
    # 我们这里的市值因子使用总市值的数值
    df[f'换手率Mean_{n}'] = (df['成交额'] / df['流通市值']).rolling(n).mean()
    factor_col = df[f'换手率Mean_{n}']
