...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Tsi', is_sort_asc, n, arg)
    含义：diff_close = 收盘价 - 昨日收盘价, ema_ema = EMA(EMA(diff_close, 2 * n), n), abs_ema_ema = EMA(EMA(abs(diff_close), 2 * n), n)
        Tsi = ema_ema / abs_ema_ema * 100
    示例：'factor_list': [
                            ('Tsi', True, 20, 1),         # Tsi_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    n1 = 2 * n
    df['diff_close'] = df['收盘价_复权'] - df['收盘价_复权'].shift(1)
    ...
    # 创建因子列
    df[f'Tsi_{n}'] = df['ema_ema'] / df['abs_ema_ema'] * 100
    factor_col = df[f'Tsi_{n}']
