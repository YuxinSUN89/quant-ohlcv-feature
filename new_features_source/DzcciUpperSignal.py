...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('DzcciUpperSignal', is_sort_asc, n, arg)
    含义：CCI = (TP - MA(TP,n)) / (0.015 *MA(TP - MA(TP,n), n)) ,其中TP = (最高价 + 最低价 + 收盘价) / 3
        CCI_middle = MA(CCI,n) , CCI_Upper = CCI_middle + 2 * STD(CCI,n)
        DzcciUpperSignal = Scale01(CCI_Lower-收盘价, n)
    示例：'factor_list': [
                            ('DzcciUpperSignal', True, 10, 1),         # DzcciUpperSignal_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    signal = df['收盘价_复权'] - cci_upper
    # ========== 原始计算逻辑结束 ==========

    # 创建因子列
    df[f'DzcciUpperSignal_{n}'] = scale_01(signal, n)
    factor_col = df[f'DzcciUpperSignal_{n}']
