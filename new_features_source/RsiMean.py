...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('RsiMean', is_sort_asc, n, arg)
    含义：diff = 收盘价.diff(), 如果diff>0, up = diff, down = 0; 如果diff<0, up = 0, down = abs(diff)
        RsiMean = MA(SUM(up, n) / (SUM(down, n) + SUM(up, n)), n)
    示例：'factor_list': [
                            ('RsiMean', True, 20, 1),         # RsiMean_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'RsiMean_{n}'] = df['rsi'].rolling(n, min_periods=1).mean()
    factor_col = df[f'RsiMean_{n}']
