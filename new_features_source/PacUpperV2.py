...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('PacUpperV2', is_sort_asc, n, arg)
    含义：PacUpperV2 = Scale01(收盘价 - EMA(最高价, n), n)
    示例：'factor_list': [
                            ('PacUpperV2', True, 20, 1),         # PacUpperV2_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    upper = df['最高价_复权'].ewm(alpha=1 / n, adjust=False).mean()
    upper = df['收盘价_复权'] - upper
    # ========== 原始计算逻辑结束 ==========

    # 创建因子列
    df[f'PacUpperV2_{n}'] = scale_01(upper, n)
    factor_col = df[f'PacUpperV2_{n}']
