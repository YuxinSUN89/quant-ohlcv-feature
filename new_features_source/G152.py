...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('G152', is_sort_asc, n, arg)
    含义：G152 = SMA(MEAN(DELAY(SMA(DELAY(CLOSE/DELAY(CLOSE,9),1),9,1),1),12)-MEAN(DELAY(SMA(DELAY(CLOSE/DELAY(CLOSE,9),1),9,1),1),26),9,1)
    示例：'factor_list': [
                            ('G152', True, '', 1),         # G152
                        ]
    """
    ...
    # ========== 原始计算逻辑开始 ==========
    df['p1'] = df['收盘价_复权'] / df['收盘价_复权'].shift(9) # CLOSE/DELAY(CLOSE,9)
    ...
    del df['p1'], df['p2'], df['p3'], df['p4'], df['p5'], df['p6'], df['p7']
    # ========== 原始计算逻辑结束 ==========
    # 创建因子列
    factor_col = df['G152']
    # 创建包含指定因子的DataFrame
