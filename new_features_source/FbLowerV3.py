...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('FbLowerV3', is_sort_asc, n, arg)
    含义：atr = MA(max((最高价-最低价), abs(最高价-昨日收盘价), abs(最低价-昨日收盘价)), n)
        FbLowerV3 = Scale01(MA(n) - 4.236 * atr, n)
    示例：'factor_list': [
                            ('FbLowerV3', True, 10, 1),         # FbLowerV3_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'FbLowerV3_{n}'] = scale_01(signal, n)
    factor_col = df[f'FbLowerV3_{n}']
