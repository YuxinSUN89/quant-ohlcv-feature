...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('TiiSignalV2', is_sort_asc, n, arg)
    含义：dev = 收盘价 - MA(n), 当dev>0时，devpos = dev, 当dev<0时，devneg = -dev
        sumpos = SUM(devpos, int(1 + n / 2 )), sumneg = SUM(devneg, int(1 + n / 2 )),
        TiiSignalV2 = scale01(100 * sumpos / (sumpos + sumneg) - EMA(100 * sumpos / (sumpos + sumneg), int(n / 2)), n)
    示例：'factor_list': [
                            ('TiiSignalV2', True, 20, 1),         # TiiSignalV2_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    close_ma = df['收盘价_复权'].rolling(n, min_periods=1).mean()
    dev = df['收盘价_复权'] - close_ma
    ...
    # 创建因子列
    df[f'TiiSignalV2_{n}'] = scale_01(tii_signal, n)
    factor_col = df[f'TiiSignalV2_{n}']
