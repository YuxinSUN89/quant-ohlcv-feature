...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('PacUpper', is_sort_asc, n, arg)
    含义：PacUpper = Scale01(EMA(最高价, n), n)
    示例：'factor_list': [
                            ('PacUpper', True, 20, 1),         # PacUpper_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'PacUpper_{n}'] = scale_01(upper, n)
    factor_col = df[f'PacUpper_{n}']
