...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('HullmaSignal', is_sort_asc, n, arg)
    含义：X = 2*EMA(CLOSE,[N/2])-EMA(CLOSE,N), HullmaSignal = Scale01(X - EMA(X,[√𝑁])), n的缺省值为20。 
    示例：'factor_list': [
                            ('HullmaSignal', True, '', 1),         # HullmaSignal_20
                            ('HullmaSignal', True, 10, 1),         # HullmaSignal_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    _x = 2 * df['收盘价_复权'].ewm(span=int(n / 2), adjust=False, min_periods=1).mean() - df['收盘价_复权'].ewm(
    ...
    # 创建因子列
    df[f'HullmaSignal_{n}'] = scale_01(signal, n)
    factor_col = df[f'HullmaSignal_{n}']
