...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Tdi', is_sort_asc, n, arg)
    含义：a = n日内上涨总和 * 1000, b = abs(n日内下跌总和) * 1000, rsi = a / (a + b)
        Tdi = Scale01(EMA(rsi, n) - EMA(rsi, 2 * n))
    示例：'factor_list': [
                            ('Tdi', True, 20, 1),         # Tdi_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'Tdi_{n}'] = scale_01(signal, n)
    factor_col = df[f'Tdi_{n}']
