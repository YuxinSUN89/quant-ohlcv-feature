...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('BollingV2', is_sort_asc, n, arg)
    含义：BollingV2是把Bolling的上下线相较于中线±2*std 改成±0.5*std
    示例：'factor_list': [
                            ('BollingV2', True, '', 1),         # BollingV2_20
                            ('BollingV2', True, 10, 1),         # BollingV2_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'BollingV2_{n}'] = (df['upper'] - df['lower']) / (df['median'] + 1e-8)
    factor_col = df[f'BollingV2_{n}']
