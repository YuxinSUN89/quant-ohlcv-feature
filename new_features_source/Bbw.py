...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    df['price_ma'] = df['收盘价_复权'].rolling(window=n, min_periods=1).mean()
    df['price_std'] = df['收盘价_复权'].rolling(window=n, min_periods=1).std(ddof=0)
    # 原本BBW是布林带(上轨-下轨) / 中轨，简化后就是 4 * std / ma
    df[f'Bbw_{n}'] = 4 * df['price_std'] / df['price_ma']
    factor_col = df[f'Bbw_{n}']

    del df['price_ma'], df['price_std']
    # 创建包含指定因子的DataFrame
