...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('SmiV2', is_sort_asc, n, arg)
    含义：DS = EMA(EMA(收盘价 - (MA(最高价, n) + MA(最低价, n)) / 2, n), n), DHL = EMA(EMA(MA(最高价, n) - MA(最低价, n), n), n)
        SmiV2 = Scale01(MA(100 * DS / DHL, n))
    示例：'factor_list': [
                            ('SmiV2', True, 20, 1),         # SmiV2_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    m = 0.5 * df['最高价_复权'].rolling(n, min_periods=1).max() + 0.5 * df['最低价_复权'].rolling(n, min_periods=1).min()
    d = df['收盘价_复权'] - m
    ds = d.ewm(span=n, adjust=False, min_periods=1).mean()
    ds = ds.ewm(span=n, adjust=False, min_periods=1).mean()
    dhl = df['最高价_复权'].rolling(n, min_periods=1).max() - df['最低价_复权'].rolling(n, min_periods=1).min()
    ...
    # 创建因子列
    df[f'SmiV2_{n}'] = scale_01(signal, n)
    factor_col = df[f'SmiV2_{n}']
