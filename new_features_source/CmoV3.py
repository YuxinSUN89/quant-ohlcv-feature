...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('CmoV3', is_sort_asc, n, arg)
    含义：CmoV3是对CMO取n日均值。n的缺省值为20。
    示例：'factor_list': [
                            ('CmoV3', True, '', 1),         # CmoV3_20
                            ('CmoV3', True, 30, 1),         # CmoV3_30
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    df['momentum'] = df['收盘价_复权'] - df['收盘价_复权'].shift(1)
    ...
    df['cmo'] = (df['up_sum'] - df['dn_sum']) / (df['up_sum'] + df['dn_sum'] + 1e-8) * 100
    # ========== 原始计算逻辑结束 ==========

    # 创建因子列
    df[f'CmoV3_{n}'] = df['cmo'].rolling(window=n, min_periods=1).mean()
    factor_col = df[f'CmoV3_{n}']
