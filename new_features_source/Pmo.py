...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Pmo', is_sort_asc, n, arg)
    含义：Pmo = MA(MA(MA((收盘价 - 昨日收盘价) / 昨日收盘价 * 100, n) * 10, 4 * n), 2 * n)
    示例：'factor_list': [
                            ('Pmo', True, 20, 1),         # Pmo_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    df['ROC'] = (df['收盘价_复权'] - df['收盘价_复权'].shift(1)) / df['收盘价_复权'].shift(1) * 100
    ...
    # 创建因子列
    df[f'Pmo_{n}'] = df['PMO'].rolling(2 * n, min_periods=1).mean()
    factor_col = df[f'Pmo_{n}']
