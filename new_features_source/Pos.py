...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Pos', is_sort_asc, n, arg)
    含义：price = (收盘价 - 收盘价.shift(n)) / 收盘价.shift(n)
        Pos = (price - MIN(price, n)) / (MAX(price, n) - MIN(price, n))
    示例：'factor_list': [
                            ('Pos', True, 20, 1),         # Pos_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    ref = df['收盘价_复权'].shift(n)
    price = (df['收盘价_复权'] - ref) / ref
    min_price = price.rolling(n).min()
    max_price = price.rolling(n).max()
    # ========== 原始计算逻辑结束 ==========

    # 创建因子列
    df[f'Pos_{n}'] = (price - min_price) / (max_price - min_price)
    factor_col = df[f'Pos_{n}']
