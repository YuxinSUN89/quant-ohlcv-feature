...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    含义：fi = 成交量 * (收盘价- 昨日收盘价), FiRsi是把RSI中的收盘价换成fi
    示例：'factor_list': [
                            ('FiRsi', True, 10, 1),         # FiRsi_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    df['_fi'] = df['成交量'] * (df['收盘价_复权'] - df['收盘价_复权'].shift(1))
    diff = df['_fi'].diff()
    ...
    # 创建因子列
    df[f'FiRsi_{n}'] = pd.Series(signal)
    factor_col = df[f'FiRsi_{n}']

    # 清理中间列（如果有）

    del  df['_fi'], df['up'], df['down']
    # 创建包含指定因子的DataFrame
