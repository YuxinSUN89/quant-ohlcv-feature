...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    示例：'factor_list': [
                            ('Fisher', True, '', 1),         # Fisher_20
                            ('Fisher', True, 10, 1),         # Fisher_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    df['price_ch'] = 2 * (df['price'] - df['min_low']) / (df['max_high'] - df['最低价_复权']) - 0.5
    ...
    # 创建因子列
    df[f'Fisher_{n}'] = 0.5 * np.log((1+df['price_change']) / (1 - df['price_change']))
    factor_col = df[f'Fisher_{n}']
