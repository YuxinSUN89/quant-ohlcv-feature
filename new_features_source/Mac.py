...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Mac', is_sort_asc, n, arg)
    含义：Mac = Scale01(10 * (MA(n) - MA(2*n)), n)
    示例：'factor_list': [
                            ('Mac', True, 20, 1),         # Mac_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    ma_short = df['收盘价_复权'].rolling(n, min_periods=1).mean()
    ma_long = df['收盘价_复权'].rolling(2 * n, min_periods=1).mean()
    _mac = 10 * (ma_short - ma_long)
    # ========== 原始计算逻辑结束 ==========

    # 创建因子列
    df[f'Mac_{n}'] = scale_01(_mac, n)
    factor_col = df[f'Mac_{n}']
