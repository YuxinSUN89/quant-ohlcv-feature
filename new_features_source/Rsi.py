...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Rsi', is_sort_asc, n, arg)
    含义：diff = 收盘价.diff(), 如果diff>0, up = diff, down = 0; 如果diff<0, up = 0, down = abs(diff)
        Rsi = EMA(up, n) / (EMA(down, n) + EMA(up, n))
    示例：'factor_list': [
                            ('Rsi', True, 20, 1),         # Rsi_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    diff = df['收盘价_复权'].diff()  # CLOSE-REF(CLOSE,1) 计算当前close 与前一周期的close的差值
    ...
    # 创建因子列
    df[f'Rsi_{n}'] = A / (A + B + 1e-8)
    factor_col = df[f'Rsi_{n}']
