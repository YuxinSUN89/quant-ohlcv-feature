...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    含义：mtm_mean = MA(收盘价 / 昨日收盘价 - 1, n), atr = MA(MAX(最高价 - 最低价, abs(最高价 - 昨日收盘价), abs(最低价 - 昨日收盘价), n)
        ...
        V1 = mtm_mean * wd_atr * mtm_atr * mtm_atr_mean, up1 = MA(V1, n) + STD(V1, n) * MA(abs(V1 - MA(V1, n)) / STD(V1, n), n)
        V1UpV2 = up1 - V1
    示例：'factor_list': [
                            ('V1UpV2', True, 20, 1),         # V1UpV2_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'V1UpV2_{n}'] = up1 - indicator
    factor_col = df[f'V1UpV2_{n}']
