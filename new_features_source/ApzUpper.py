...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    用了最高价与最低价差值的 N 日双重指数平均来反映价格的波动幅度。这里取Apz的上轨并进行归一化处理，N缺省值为10
    VOL=EMA(EMA(HIGH-LOW,N),N)
    UPPER=EMA(EMA(CLOSE,M),M)+PARAM*VOL
    LOWER=EMA(EMA(CLOSE,M),M)-PARAM*VOL
    示例：'factor_list': [
                            ('ApzUpper', True, '', 1),         # ApzUpper_10
                            ('ApzUpper', True, 20, 1),         # ApzUpper_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'ApzUpper_{n}'] = scale_01(signal, n)
    factor_col = df[f'ApzUpper_{n}']
