...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('CmoV2', is_sort_asc, n, arg)
    含义：CmoV2与CMO相比，是用过去N天的最大价格上涨量和最大价格下跌量（取绝对值）得到。n的缺省值为20。
    示例：'factor_list': [
                            ('CmoV2', True, '', 1),         # CmoV2_20
                            ('CmoV2', True, 30, 1),         # CmoV2_30
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'CmoV2_{n}'] = (df['up_sum'] - df['dn_sum']) / (df['up_sum'] + df['dn_sum'] + 1e-8)
    factor_col = df[f'CmoV2_{n}']
