...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Adxrneg', is_sort_asc, n, arg)
    含义：n缺省值为14。对Adxr取归一化
    示例：'factor_list': [  
                            ('Adxrneg', True, '', 1),       # Adxrneg_14    
                            ('Adxrneg', True, 10, 1),       # Adxrneg_10    
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'Adxrneg_{n}'] = scale_01(signal, n)
    factor_col = df[f'Adxrneg_{n}']
