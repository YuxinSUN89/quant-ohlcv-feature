...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    # ========== 原始计算逻辑开始 ==========
    df['G159_delay'] = df['收盘价_复权'].shift(1)
    df['G159_min_value'] = df.loc[:, ['最低价_复权', 'G159_delay']].min(axis=1)
    df['G159_max_value'] = df.loc[:, ['最高价_复权', 'G159_delay']].max(axis=1)
    ...
    df['G159'] = ((df['收盘价_复权'] - df['G159_sum_min_6']) / df['G159_sum_6'] * 12 * 24 + ( df['收盘价_复权'] - df['G159_sum_min_12']) / df['G159_sum_12'] * 6 * 24 + ( df['收盘价_复权'] - df['G159_sum_min_24']) / df['G159_sum_24'] * 6 * 24) * 100 / ( 6 * 12 + 6 * 24 + 12 * 24)
    del df['G159_delay'], df['G159_min_value'], df['G159_max_value'], df['G159_sum_min_6'], df['G159_sum_min_12'], df['G159_sum_min_24'],df['G159_sum_12'], df['G159_sum_24']
    # ========== 原始计算逻辑结束 ==========
    # 创建因子列
    factor_col = df['G159']
    # 创建包含指定因子的DataFrame
