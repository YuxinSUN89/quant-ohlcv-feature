...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('G96', is_sort_asc, n, arg)
    含义：G96 = SMA(SMA((CLOSE-TSMIN(LOW,9))/(TSMAX(HIGH,9)-TSMIN(LOW,9))*100,3,1),3,1)
    示例：'factor_list': [
                            ('G96', True, '', 1),         # G96
                        ]
    """
    ...
    # ========== 原始计算逻辑开始 ==========
    df['G96_prepare_1'] = ((df['收盘价_复权'] - df['最低价_复权'].rolling(9).min()) / ( df['最高价_复权'].rolling(9).max() - df['最低价_复权'].rolling(9).min())) * 100
    df['G96_prepare_2'] = df['G96_prepare_1'].ewm(alpha=1 / 3, adjust=False).mean()
    df['G96'] = df['G96_prepare_2'].ewm(alpha=1 / 3, adjust=False).mean()
    del df['G96_prepare_1'], df['G96_prepare_2']
    # ========== 原始计算逻辑结束 ==========
    # 创建因子列
    factor_col = df['G96']
    # 创建包含指定因子的DataFrame
