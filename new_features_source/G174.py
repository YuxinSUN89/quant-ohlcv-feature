...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('G174', is_sort_asc, n, arg)
    含义：G174 = SMA((CLOSE>DELAY(CLOSE,1)?STD(CLOSE,20):0),20,1)
    示例：'factor_list': [
                            ('G174', True, '', 1),         # G174
                        ]
    """
    ...
    # ========== 原始计算逻辑开始 ==========
    df['G174_pre'] = np.where(df['收盘价_复权'] > df['收盘价_复权'].shift(1), df['收盘价_复权'].rolling(20).std(), 0)
