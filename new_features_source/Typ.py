...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Typ', is_sort_asc, n, arg)
    含义：dif_typ = EMA((收盘价 + 最高价 + 最低价) / 3, n) - EMA((收盘价 + 最高价 + 最低价) / 3, 3 * n)
        Typ = dif_typ - MA(dif_typ, n) / STD(dif_typ, n)
    示例：'factor_list': [
                            ('Typ', True, 20, 1),         # Typ_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    TYP = (df['收盘价_复权'] + df['最高价_复权'] + df['最低价_复权']) / 3
    ...
    # 创建因子列
    df[f'Typ_{n}'] = diff_TYP - diff_TYP_mean / diff_TYP_std
    factor_col = df[f'Typ_{n}']
