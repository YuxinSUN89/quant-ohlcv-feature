...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('MaSignal', is_sort_asc, n, arg)
    含义：MaSignal = Scale01(收盘价 - MA(n))
    示例：'factor_list': [
                            ('MaSignal', True, 20, 1),         # MaSignal_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    signal = df['收盘价_复权'] - df['收盘价_复权'].rolling(n, min_periods=1).mean()
    # ========== 原始计算逻辑结束 ==========

    # 创建因子列
    df[f'MaSignal_{n}'] = scale_01(signal, n)
    factor_col = df[f'MaSignal_{n}']
