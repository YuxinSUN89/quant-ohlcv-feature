...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Grid', is_sort_asc, n, arg)
    含义：grid = MA((收盘价 - MA(n))/STD(n), n), Grid = grid.change(n)
    示例：'factor_list': [
                            ('Grid', True, 10, 1),         # Grid_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    df['grid'] = (df['收盘价_复权'] - df['median']) / df['std']
    ...
    # 创建因子列
    df[f'Grid_{n}'] = df['grid'].pct_change(n)
    factor_col = df[f'Grid_{n}']

    # 清理中间列（如果有）
    del df['median'], df['std'], df['grid']
