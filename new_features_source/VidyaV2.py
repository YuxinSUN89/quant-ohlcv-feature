...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('VidyaV2', is_sort_asc, n, arg)
    含义：TS = (开盘价 + 收盘价) / 2
        VI = abs(TS - TS.shift(n) / SUM(abs(TS - TS.shift(1)), n)
        VidyaV2 = TS / (VI * TS + (1 - VI) * TS.shift(1))
    示例：'factor_list': [
                            ('VidyaV2', True, 20, 1),         # VidyaV2_20
                        ]
    """
    # 从额外参数中获取因子名称
