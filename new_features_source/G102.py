...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('G102', is_sort_asc, n, arg)
    含义：G102 = SMA(MAX(VOLUME-DELAY(VOLUME,1),0),6,1)/SMA(ABS(VOLUME-DELAY(VOLUME,1)),6,1)*100
    示例：'factor_list': [
                            ('G102', True, '', 1),         # G102
                        ]
    """
    ...
    # ========== 原始计算逻辑开始 ==========
    df['volume_diff'] = df['成交量'] - df['成交量'].shift(1)
    ...
    del df['volume_diff'], df['max'], df['smamax'], df['smaabs']
    # ========== 原始计算逻辑结束 ==========
    # 创建因子列
    factor_col = df['G102']
    # 创建包含指定因子的DataFrame
