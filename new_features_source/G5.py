...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('G5', is_sort_asc, n, arg)
    含义：G5 = -1 * TSMAX(CORR(TSRANK(VOLUME, 5), TSRANK(HIGH, 5), 5), 3)
    示例：'factor_list': [
                            ('G5', True, '', 1),         # G5
                        ]
    """
    ...
    df['G5'] = -1 * (((df['成交量'].rolling(5).apply(TS_RANK)).rolling(5).corr(df['最高价_复权'].rolling(5).apply(TS_RANK)))
