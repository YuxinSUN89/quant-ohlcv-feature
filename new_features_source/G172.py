...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('G172', is_sort_asc, n, arg)
    含义：G172 = MEAN(ABS(SUM((LD>0 & LD>HD)?LD:0,14)*100/SUM(TR,14)-SUM((HD>0 & HD>LD)?HD:0,14)*100/SUM(TR,14))/(SUM((LD>0 & LD>HD)?LD:0,14)*100/SUM(TR,14)+SUM((HD>0 & HD>LD)?HD:0,14)*100/SUM(TR,14))*100,6)
    示例：'factor_list': [
                            ('G172', True, '', 1),         # G172
                        ]
    """
    ...
    # ========== 原始计算逻辑开始 ==========
    df['G172_TR_MAX1_PRE'] = df['最高价_复权'] - df['最低价_复权']
    df['G172_TR_MAX1_AFTER'] = abs(df['最高价_复权'] - df['收盘价_复权'].shift())
    df['G172_TR_MAX1'] = df[['G172_TR_MAX1_PRE', 'G172_TR_MAX1_AFTER']].max(axis=1)
    df['G172_TR_MAX2_AFTER'] = abs(df['最低价_复权'] - df['收盘价_复权'].shift())
    df['G172_TR'] = df[['G172_TR_MAX1', 'G172_TR_MAX2_AFTER']].max(axis=1)
    df['G172_LD'] = - df['最低价_复权'].diff()
    df['G172_HD'] = df['最高价_复权'].diff()
    ...
    del df['G172_TR_MAX1_PRE'], df['G172_TR_MAX1_AFTER'], df['G172_TR_MAX1'], df['G172_TR_MAX2_AFTER'], df['G172_TR'], df['G172_LD'],df['G172_LD_HD'], df['G172_HD_LD'], df['G172_part1'], df['G172_part2'], df['G172_MEAN']
    # ========== 原始计算逻辑结束 ==========
    # 创建因子列
    factor_col = df['G172']
    # 创建包含指定因子的DataFrame
