...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('DzcciUpperSignalV2', is_sort_asc, n, arg)
    含义：CCI = (TP - MA(TP,n)) / (0.015 *MA(TP - MA(TP,n), n)) ,其中TP = (最高价 + 最低价 + 收盘价) / 3
        CCI_middle = MA(CCI,n) , CCI_Upper = CCI_middle + 2 * STD(CCI,n), CCI_MA = MA(CCI, n/4)
        DzcciUpperSignalV2 = Scale01(CCI_MA - CCI_Lower, n)
    示例：'factor_list': [
                            ('DzcciUpperSignalV2', True, 10, 1),         # DzcciUpperSignalV2_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'DzcciUpperSignalV2_{n}'] = scale_01(signal, n)
    factor_col = df[f'DzcciUpperSignalV2_{n}']
