...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('G173', is_sort_asc, n, arg)
    含义：G173 = MEAN(ABS(SUM((LD>0 & LD>HD)?LD:0,14)*100/SUM(TR,14)-SUM((HD>0 & HD>LD)?HD:0,14)*100/SUM(TR,14))/(SUM((LD>0 & LD>HD)?LD:0,14)*100/SUM(TR,14)+SUM((HD>0 & HD>LD)?HD:0,14)*100/SUM(TR,14))*100,6)
    示例：'factor_list': [
                            ('G173', True, '', 1),         # G173
                        ]
    """
    ...
    # ========== 原始计算逻辑开始 ==========
    df['a'] = df['收盘价_复权'].ewm(alpha=2 / 13, adjust=False).mean()
    df['b'] = df['a'].ewm(alpha=2 / 13, adjust=False).mean()
    df['c'] = np.log(df['收盘价_复权'])
