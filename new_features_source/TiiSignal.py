...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('TiiSignal', is_sort_asc, n, arg)
    含义：dev = 收盘价 - MA(n), 当dev>0时，devpos = dev, 当dev<0时，devneg = -dev
        sumpos = SUM(devpos, int(1 + n / 2 )), sumneg = SUM(devneg, int(1 + n / 2 )),
        TiiSignal = Zscore(EMA(100 * sumpos / (sumpos + sumneg), int(n / 2)), n)
    示例：'factor_list': [
                            ('TiiSignal', True, 20, 1),         # TiiSignal_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'TiiSignal_{n}'] = scale_zscore(tii_signal, n)
    factor_col = df[f'TiiSignal_{n}']
