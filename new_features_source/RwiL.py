...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('RwiL', is_sort_asc, n, arg)
    含义：atr = MA(MAX(abs(最高价 - 最低价), abs(最高价 - 昨日收盘价), abs(昨日收盘价 - 最低价)) , n)
        RwiL = (昨日收盘价 - 最低价) / (n^0.5 * atr), n的缺省值为14
    示例：'factor_list': [
                            ('RwiL', True, '', 1),         # RwiL_14
                            ('RwiL', True, 20, 1),         # RwiL_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'RwiL_{n}'] = (df['最高价_复权'] - df['最低价_复权'].shift(1)) / (df['ATR'] * np.sqrt(n))
    factor_col = df[f'RwiL_{n}']
