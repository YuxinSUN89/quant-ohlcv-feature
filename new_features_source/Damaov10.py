...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Damaov10', is_sort_asc, n, arg)
    含义：Damaov10 = RC均值 * BBW均值 * ATR
    示例：'factor_list': [
                            ('Damaov10', True, 10, 1),         # Damaov10_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    df['RC'] = 100 * ((df['收盘价_复权'] - df['收盘价_复权'].shift(n)) / df['收盘价_复权'].shift(n) + (df['收盘价_复权'] - df['收盘价_复权'].shift(2 * n)) / df['收盘价_复权'].shift(2 * n))
    df['RC_mean'] = df['RC'].rolling(n, min_periods=1).mean()
    df['median'] = df['收盘价_复权'].rolling(window=n).mean()
    df['std'] = df['收盘价_复权'].rolling(n, min_periods=1).std(ddof=0)
    df['z_score'] = abs(df['收盘价_复权'] - df['median']) / df['std']
    ...
    # 创建因子列
    df[f'Damaov10_{n}'] = df['RC_mean'] * df['BBW_mean'] * df['ATR']
    factor_col = df[f'Damaov10_{n}']
