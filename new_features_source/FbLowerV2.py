...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('FbLowerV2', is_sort_asc, n, arg)
    含义：atr = MA(max((最高价-最低价), abs(最高价-昨日收盘价), abs(最低价-昨日收盘价)), n)
        FbLowerV2 = Scale01(MA(n) - 2.618 * atr, n)
    示例：'factor_list': [
                            ('FbLowerV2', True, 10, 1),         # FbLowerV2_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'FbLowerV2_{n}'] = scale_01(signal, n)
    factor_col = df[f'FbLowerV2_{n}']
