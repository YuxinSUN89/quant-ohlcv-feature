...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Nvi', is_sort_asc, n, arg)
    含义：如果缩量, nvi_inc = 1 + (收盘价 - 昨日收盘价) / 收盘价; 其它情况, nvi_inc = 1。 nvi的第一个值为100，后面依次累乘nvi_inc
        Nvi = Scale01(nvi - MA(nvi, n), n) 
    示例：'factor_list': [
                            ('Nvi', True, 20, 1),         # Nvi_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    nvi_inc = np.where(df['成交量'] < df['成交量'].shift(1), 1 + (df['收盘价_复权'] - df['收盘价_复权'].shift(1)) / (1e-9 + df['收盘价_复权']), 1)
    nvi_inc[0] = 100
    ...
    # 创建因子列
    df[f'Nvi_{n}'] = scale_01(signal, n)
    factor_col = df[f'Nvi_{n}']
