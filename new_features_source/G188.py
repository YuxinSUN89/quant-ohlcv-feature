...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('G188', is_sort_asc, n, arg)
    含义：G188 = ((HIGH-LOW–SMA(HIGH-LOW,11,2))/SMA(HIGH-LOW,11,2))*100
    示例：'factor_list': [
                            ('G188', True, '', 1),         # G188
                        ]
    """
    ...
    # ========== 原始计算逻辑开始 ==========
    df["HIGH_LOW_DIFF"] = df["最高价_复权"] - df["最低价_复权"]
    ...
    del df["HIGH_LOW_DIFF"],df["SMA_11_2_VALUE"]
    # ========== 原始计算逻辑结束 ==========
    # 创建因子列
    factor_col = df['G188']
    # 创建包含指定因子的DataFrame
