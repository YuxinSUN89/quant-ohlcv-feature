...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('G109', is_sort_asc, n, arg)
    含义：G109 = SMA(HIGH-LOW,10,2)/SMA(SMA(HIGH-LOW,10,2),10,2)
    示例：'factor_list': [
                            ('G109', True, '', 1),         # G109
                        ]
    """
    ...
    # ========== 原始计算逻辑开始 ==========
    df['G109_SMA_1'] = (df['最高价_复权'] - df['最低价_复权']).ewm(alpha=(2 / 10), adjust=False).mean()
    df['G109_SMA_2'] = df['G109_SMA_1'].ewm(alpha=(2 / 10), adjust=False).mean()
    df['G109'] = df['G109_SMA_1'] / df['G109_SMA_2']
    del df['G109_SMA_1'], df['G109_SMA_2']
    # ========== 原始计算逻辑结束 ==========
    # 创建因子列
    factor_col = df['G109']
    # 创建包含指定因子的DataFrame
