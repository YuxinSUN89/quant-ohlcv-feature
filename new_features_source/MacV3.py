...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('MacV3', is_sort_asc, n, arg)
    含义：MacV3 = Scale01(10 * (MA(0.5 * MA(最高价,n) + 0.5 * MA(最低价,n), n) - MA(0.5 * MA(最高价,n) + 0.5 * MA(最低价,n), 2*n)), n)
    示例：'factor_list': [
                            ('MacV3', True, 20, 1),         # MacV3_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'MacV3_{n}'] = scale_01(_mac, n)
    factor_col = df[f'MacV3_{n}']
