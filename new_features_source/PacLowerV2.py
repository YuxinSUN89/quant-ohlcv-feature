...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('PacLowerV2', is_sort_asc, n, arg)
    含义：PacLowerV2 = Scale01(EMA(最低价, n) - 收盘价, n)
    示例：'factor_list': [
                            ('PacLowerV2', True, 20, 1),         # PacLowerV2_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    lower = df['最低价_复权'].ewm(alpha=1 / n, adjust=False).mean()
    lower = lower - df['收盘价_复权']
    # ========== 原始计算逻辑结束 ==========

    # 创建因子列
    df[f'PacLowerV2_{n}'] = scale_01(lower, n)
    factor_col = df[f'PacLowerV2_{n}']
