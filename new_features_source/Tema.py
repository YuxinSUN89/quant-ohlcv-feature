...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Tema', is_sort_asc, n, arg)
    含义：Tema = EMA(n) / (3 * EMA(n) - 3 * EMA(EMA(n), n) + EMA(EMA(EMA(n), n), n))
    示例：'factor_list': [
                            ('Tema', True, 20, 1),         # Tema_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'Tema_{n}'] = df['ema'] / (df['TEMA'] + 1e-8) - 1
    factor_col = df[f'Tema_{n}']
