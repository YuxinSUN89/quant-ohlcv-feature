...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('PacLower', is_sort_asc, n, arg)
    含义：PacLower = Scale01(EMA(最低价, n), n)
    示例：'factor_list': [
                            ('PacLower', True, 20, 1),         # PacLower_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'PacLower_{n}'] = scale_01(lower, n)
    factor_col = df[f'PacLower_{n}']
