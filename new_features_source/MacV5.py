...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('MacV5', is_sort_asc, n, arg)
    含义：TP = (MAX(最高价,n) + MIN(最低价,n) + 开盘价) / 3,
        MacV5 = Scale01(10 * (MA(TP,n) - MA(TP, 2*n)), n)
    示例：'factor_list': [
                            ('MacV5', True, 20, 1),         # MacV5_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'MacV5_{n}'] = scale_01(_mac, n)
    factor_col = df[f'MacV5_{n}']
