...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Bolling', is_sort_asc, n, arg)
    含义：Bolling = (upper - lower) / midian
    示例：'factor_list': [
                            ('Bolling', True, '', 1),         # Bolling_20
                            ('Bolling', True, 10, 1),         # Bolling_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'Bolling_{n}'] = (df['upper'] - df['lower']) / (df['median'] + 1e-8)
    factor_col = df[f'Bolling_{n}']
