...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Rsj', is_sort_asc, n, arg)
    含义：如果收盘价上涨，positive_data = 收盘价 / 昨日收盘价 - 1, negative_data = 0; 如果收盘价下跌，positive_data = 0, negative_data = 收盘价 / 昨日收盘价 - 1
        rv = SUM((收盘价 / 昨日收盘价 - 1)^2, n), rv+ = SUM(positive_data^2, n)， rv- = SUM(negative_data^2, n)
        Rsj = (rv+ - rv-) / rv 
    示例：'factor_list': [
                            ('Rsj', True, 20, 1),         # Rsj_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    # 计算收益率
    df['return'] = df['收盘价_复权'] / df['收盘价_复权'].shift(1) - 1
    ...
    # 创建因子列
    df[f'Rsj_{n}'] = (df['rv+'] - df['rv-']) / (df['rv'] + 1e-8)
    factor_col = df[f'Rsj_{n}']

    # 清理中间列（如果有）

    del  df['pow_return'], df['pow_positive_data'], df['pow_negetive_data'],  df['positive_data'], df['negative_data'],  df['rv+'], df['rv-'],  df['rv']
    # 创建包含指定因子的DataFrame
