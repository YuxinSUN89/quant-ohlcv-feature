...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    用了最高价与最低价差值的 N 日双重指数平均来反映价格的波动幅度。 N缺省值为10
    示例：'factor_list': [
                            ('Apz', True, '', 1),         # Apz_10
                            ('Apz', True, 20, 1),         # Apz_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    df['ema_close'] = df['收盘价_复权'].ewm(2 * n, adjust=False).mean()
    df['ema_ema_close'] = df['ema_close'].ewm(2 * n, adjust=False).mean()
    # ========== 原始计算逻辑结束 ==========

    # 创建因子列
    df[f'Apz_{n}'] = df['vol'] / df['ema_ema_close']
    factor_col = df[f'Apz_{n}']


    del df['hl'],  df['ema_hl'], df['vol'], df['ema_close'], df['ema_ema_close']
    # 创建包含指定因子的DataFrame
