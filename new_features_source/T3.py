...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('T3', is_sort_asc, n, arg)
    含义：T1_ema = EMA(n) * 1.5 + EMA(EMA(n), n) * 0.5, T2_ema = EMA(T1_ema * 1.5 + EMA(T1_ema, n) * 0.5, n)
        T3 = 收盘价 / (T2_ema * 1.5 + EMA(T2_ema, n) * 0.5) -  -1
    示例：'factor_list': [
                            ('T3', True, 20, 1),         # T3_20
                        ]
    """
    ...
    # 创建因子列
    df[f'T3_{n}'] = df['收盘价_复权'] / (T3 + 1e-8) - 1
    factor_col = df[f'T3_{n}']
