...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    含义：PARAM=0.5, PRICE=(HIGH+LOW)/2
        ...
        FISHER_V2=0.5*REF(FISHER,1)+0.5*log((1+PRICE_CHANGE)/(1-PRICE_CHANGE))
    示例：'factor_list': [
                            ('FisherV2', True, '', 1),         # FisherV2_20
                            ('FisherV2', True, 10, 1),         # FisherV2_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'FisherV2_{n}'] = 0.3 * df['price_change'] + 0.7 * df['price_change'].shift(1)
    factor_col = df[f'FisherV2_{n}']
