...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('G169', is_sort_asc, n, arg)
    含义：G169 = SMA(MEAN(DELAY(SMA(CLOSE-DELAY(CLOSE,1),9,1),1),12)-MEAN(DELAY(SMA(CLOSE-DELAY(CLOSE,1),9,1),1), 26),10,1)
    示例：'factor_list': [
                            ('G169', True, '', 1),         # G169
                        ]
    """
    ...
    # ========== 原始计算逻辑开始 ==========
    df['DSCDC'] = ((df['收盘价_复权'] - df['收盘价_复权'].shift(1)).ewm(alpha=1.0 / 9, adjust=False).mean()).shift(1)
