...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('MtmMax', is_sort_asc, n, arg)
    含义：MtmMax = (收盘价 / 昨日收盘价 - 1) - MAX((收盘价 / 昨日收盘价 - 1), n).shift(1)
    示例：'factor_list': [
                            ('MtmMax', True, 20, 1),         # MtmMax_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'MtmMax_{n}'] = df['mtm'] - df['up']
    factor_col = df[f'MtmMax_{n}']
