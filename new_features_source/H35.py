...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('H35', is_sort_asc, n, arg)
    含义：H35 = ((ts_rank(VOLUME, 32) * (1 - ts_rank(((CLOSE + HIGH) - LOW), 16))) * (1 - ts_rank(RETURNS, 32)))
    示例：'factor_list': [
                            ('H35', True, '', 1),         # H35
                        ]
    """
    ...
    df['H35_to_rank_1'] = df['成交额']
    df['H35_to_rank_2'] = (df['收盘价_复权'] + df['最高价_复权']) - df['最低价_复权']
    df['H35_to_rank_3'] = (df['收盘价_复权'] - df['收盘价_复权'].shift()) / df['收盘价_复权'].shift()
    ...
    # ===== 因子计算结束 =====
    del df['H35_to_rank_1'], df['H35_to_rank_2'], df['H35_to_rank_3'], df['H35_rank_1'], df['H35_rank_2'], df['H35_rank_3']
    # 返回因子DataFrame
    return pd.DataFrame({col_name: df['H35']}, index=df.index)
