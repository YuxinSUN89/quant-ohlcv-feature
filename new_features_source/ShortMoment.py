...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('ShortMoment', is_sort_asc, n, arg)
    含义：ShortMoment 是在n日内找出振幅最小30%时间的涨跌幅相加
    示例：'factor_list': [
                            ('ShortMoment', True, 20, 1),         # ShortMoment_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'ShortMoment_{n}'] = df['涨跌幅'].rolling(n).apply(range_plus, args=(np_tmp, n, 0.7), raw=False)

    factor_col = df[f'ShortMoment_{n}']
