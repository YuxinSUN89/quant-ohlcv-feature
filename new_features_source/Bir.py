...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Bir', is_sort_asc, n, arg)
    含义：太复杂，自己看下面公式吧^_^
    示例：'factor_list': [
                            ('Bir', True, 10, 1),       # Bir_10    
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    df["p"] = (df["开盘价_复权"] + df["最高价_复权"] + df["最低价_复权"] + df["收盘价_复权"]) / 4
    ...
    # 创建因子列
    df[f'Bir_{n}'] = (df["up"] + df["down"]).rolling(short_period, min_periods=1).mean()
    factor_col = df[f'Bir_{n}']
