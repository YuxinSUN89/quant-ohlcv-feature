...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('Rccd', is_sort_asc, n, arg)
    含义：ARC1 = MA(收盘价 / 收盘价.shift(2 * n), 2 * n), DIF = MA(ARC1, n) - MA(ARC1, 2 * n)
        Rccd = MA(DIF, 2 * n)
    示例：'factor_list': [
                            ('Rccd', True, 20, 1),         # Rccd_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # ========== 原始计算逻辑开始 ==========
    df['RC'] = df['收盘价_复权'] / df['收盘价_复权'].shift(2 * n)
    ...
    # 创建因子列
    df[f'Rccd_{n}'] = df['DIF'].rolling(2 * n, min_periods=1).mean()
    factor_col = df[f'Rccd_{n}']
