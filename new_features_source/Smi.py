...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    # ========== 原始计算逻辑开始 ==========
    df['max_high'] = df['最高价_复权'].rolling(n, min_periods=1).mean()
    df['min_low'] = df['最低价_复权'].rolling(n, min_periods=1).mean()
    ...
    # 创建因子列
    df[f'Smi_{n}'] = df['SMI'].rolling(n, min_periods=1).mean()
    factor_col = df[f'Smi_{n}']
