...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    # ========== 原始计算逻辑开始 ==========
    n = 20
    df['part1'] = 16 * ( df['收盘价_复权'] - df['收盘价_复权'].shift() + (df['收盘价_复权'] - df['开盘价_复权']) / 2 + df['收盘价_复权'].shift() - df[ '开盘价_复权'].shift())
    condition1 = abs(df['最高价_复权'] - df['收盘价_复权'].shift()) > abs(df['最低价_复权'] - df['收盘价_复权'].shift())
    condition2 = abs(df['最高价_复权'] - df['收盘价_复权'].shift()) > abs(df['最高价_复权'] - df['最低价_复权'].shift())
    df['value1'] = abs(df['最高价_复权'] - df['收盘价_复权'].shift()) + \
                   abs(df['最低价_复权'] - df['收盘价_复权'].shift()) / 2 + \
                   abs(df['收盘价_复权'].shift() - df['开盘价_复权'].shift()) / 4
    df['part2'] = np.where(condition1 & condition2, df['value1'], None)
    condition3 = abs(df['最低价_复权'] - df['收盘价_复权'].shift()) > abs(df['最高价_复权'] - df['最低价_复权'].shift())
    condition4 = abs(df['最低价_复权'] - df['收盘价_复权'].shift()) > abs(df['最高价_复权'] - df['收盘价_复权'].shift())
    condition5 = df['part2'].isnull()
    df['value2'] = abs(df['最低价_复权'] - df['收盘价_复权'].shift()) + \
                   abs(df['最高价_复权'] - df['收盘价_复权'].shift()) / 2 + \
                   abs(df['收盘价_复权'].shift() - df['开盘价_复权'].shift()) / 4
    df.loc[condition3 & condition4 & condition5, 'part2'] = df['value2']
    df['value3'] = abs(df['最高价_复权'] - df['最低价_复权'].shift()) + \
                   abs(df['收盘价_复权'].shift() - df['开盘价_复权'].shift()) / 4
    df.loc[condition5, 'part2'] = df['value3']
    df['v4'] = abs(df['最高价_复权'] - df['收盘价_复权'].shift())
    df['v5'] = abs(df['最低价_复权'] - df['收盘价_复权'].shift())
    ...
    df.drop(['part1', 'part2', 'part3', 'value1', 'value2', 'value3', 'v4', 'v5', 'alpha'], axis=1, inplace=True)
    # ========== 原始计算逻辑结束 ==========
    # 创建因子列
    factor_col = df['G55']
    # 创建包含指定因子的DataFrame
