...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('DzcciLower', is_sort_asc, n, arg)
    含义：CCI = (TP - MA(TP,n)) / (0.015 *MA(TP - MA(TP,n), n)) ,其中TP = (最高价 + 最低价 + 收盘价) / 3
        CCI_middle = MA(CCI,n) , CCI_Lower = CCI_middle - 2 * STD(CCI,n)
        DzcciLower = Scale01(CCI_Lower, n)
    示例：'factor_list': [
                            ('DzcciLower', True, 10, 1),         # DzcciLower_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'DzcciLower_{n}'] = scale_01(signal, n)
    factor_col = df[f'DzcciLower_{n}']
