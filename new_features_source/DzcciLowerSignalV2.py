...
"""
import pandas as pd
import numpy as np  # 财务因子列：此列表用于存储财务因子相关的列名称
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('DzcciLowerSignalV2', is_sort_asc, n, arg)
    含义：CCI = (TP - MA(TP,n)) / (0.015 *MA(TP - MA(TP,n), n)) ,其中TP = (最高价 + 最低价 + 收盘价) / 3
        CCI_middle = MA(CCI,n) , CCI_Lower = CCI_middle - 2 * STD(CCI,n), CCI_MA = MA(CCI, n/4)
        DzcciLowerSignalV2 = Scale01(CCI_Lower - CCI_MA, n)
    示例：'factor_list': [
                            ('DzcciLowerSignalV2', True, 10, 1),         # DzcciLowerSignalV2_10
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    tp = df[['最高价_复权', '最低价_复权', '收盘价_复权']].sum(axis=1) / 3.
    ...
    # 创建因子列
    df[f'DzcciLowerSignalV2_{n}'] = scale_01(signal, n)
    factor_col = df[f'DzcciLowerSignalV2_{n}']
