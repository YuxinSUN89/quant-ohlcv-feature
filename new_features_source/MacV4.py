...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('MacV4', is_sort_asc, n, arg)
    含义：TP = (MAX(最高价,n) + MIN(最低价,n) + 收盘价) / 3,
        MacV4 = Scale01(10 * (MA(TP,n) - MA(TP, 2*n)), n)
    示例：'factor_list': [
                            ('MacV4', True, 20, 1),         # MacV4_20
                        ]
    """
    # 从额外参数中获取因子名称
    ...
    # 创建因子列
    df[f'MacV4_{n}'] = scale_01(_mac, n)
    factor_col = df[f'MacV4_{n}']
