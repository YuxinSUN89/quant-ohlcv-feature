...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> pd.DataFrame:
    ...
    """    
    ----->>>  配置方法  <<<-----
    配置：('H92', is_sort_asc, n, arg)
    含义：H92 = min(ts_rank(decay_linear(((((HIGH + LOW) / 2) + CLOSE) < (LOW + OPEN)), 14.7221), 18.8683),ts_rank(decay_linear(correlation(rank(LOW), rank(ADV30), 7.58555), 6.94024), 6.80584))
    示例：'factor_list': [
                            ('H92', True, '', 1),         # H92
                        ]
    """
    ...
    factor_col = df[f'H92']
    del df['Demax'], df['Demin'], df['Demax_ma'], df['Demin_ma']
    # 创建包含指定因子的DataFrame
