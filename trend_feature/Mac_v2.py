...
def signal(df, n, factor_name, config):
    ...
    # Formula: PRICE = (HIGH+LOW)/2; MAC = 10 * (MA(PRICE,N) - MA(PRICE,2N)); result = scale_01(MAC,N, config.normalize_eps, config=config)
    ...
    ma_short = (0.5 * df["high"] + 0.5 * df["low"]).rolling(n, min_periods=config.min_periods).mean()
    ma_long = (0.5 * df["high"] + 0.5 * df["low"]).rolling(2 * n, min_periods=config.min_periods).mean()
